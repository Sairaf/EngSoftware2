...
require_once(dirname(__FILE__).'/fachada/fachada_tipoCota.php');
//require_once(dirname(__FILE__).'/fachada/fachada_cursoBolsista.php');
...
//selecao ------------------------------------------------------
	...
	function getSelecaoPorPesqAnoEdital($codPesq){
		$fSelecao   = new fachada_selecao();
		$vetSelecao = $fSelecao->getSelecaoPorPesqAnoEdital($codPesq);
		return $vetSelecao;
	}
...
//plano --------------------------------------------------------
	...
	function getPlanoDispCodPesqAnoGeral($codPesq,$ano){
		$fPlano = new fachada_plano();
		$vetPlano = $fPlano->getPlanoDispCodPesqAnoGeral($codPesq,$ano);
		return $vetPlano;
	}
	...
	function podeCriarPlano($codPesq,$ano,$tpSelecao){
		...
		if ($vBolsaSelecao){		
			...
			if($_SESSION['sEdital'] == 1 or $_SESSION['sEdital'] == 2) $fSelecao->updateNumBolsa($numBolsaSelecao,$codPesq,$tpSelecao);			
			else $fSelecao->atualizaNumBolsas($numBolsaSelecao,$codPesq,$tpSelecao,$oSelecao->getCod());	
									
			if($tpSelecao != 17){						
				if($numBolsaSelecao+1>=$numPlanosLivres && $cont == 0){						
				//if($numBolsaSelecao>$numPlanosLivres && $cont == 0){									
					$tipoBolsa = $fBolsa->conversaoBolsaEdital($_SESSION['sEdital']);										
					$bolsas = $fBolsa->getBolsaTipoPesqAnoPlano($codPesq, $tipoBolsa,$ano);
						
					if(!$bolsas) $qtBolsa= 0;
					else $qtBolsa = count($bolsas);
				
					$totalPlanos = $numPlanosLivres + $qtBolsa;
				
					if($totalPlanos < $MAX_PLANOS) $plano = "podeCriar";
					else $plano = "naoPodeCriar";
				}else $plano = "naoPodeCriar";				
			}else $plano = "PodeCriar";
...
//categ cota
	...
	function getTipoCota(){
		$fTpEdital = new fachada_tipoCota();
		$vetTipoEdital = $fTpEdital->getTipoCota();
		return $vetTipoEdital;
	}
	/*
	function getTipoCota($codBolsa){
		$fCot = new fachada_tipoCota();
		$oCot = $fCot->getTipoCota($codBolsa);
		return $oCot;
	}
	
	function getTipoCotaCod($cod){
		$fCot = new fachada_tipoCota();
		$oCot = $fCot->getTipoCotaCod($cod);
		return $oCot;
	}
	
	function getDescTipoCotaCod($cod){
		$fCot = new fachada_tipoCota();
		$oCot = $fCot->getDescTipoCotaCod($cod);
		return $oCot;
	}*/
//fim Cota
...
//fim categ relatorio
	...
	function selecaoEdital($pesqId, $editalSelec){
		$fSel = new fachada_selecao();
		$fPer = new fachada_periodo();
		$qtdSelecao = $fSel->getSelecaoPorPesqAno($pesqId);			
		$anoSelecao = Date('Y');
		$numEditaisParticipando = 0;
		
		$oPeriodoIn     = $fPer->getInscricaoAberta();
		if($oPeriodoIn){
			$tpSelecao  = $oPeriodoIn->getTpSelecao();				
		}
		//Verifica se existe o usuário ou edital no ano atual
		while(!$qtdSelecao AND $anoSelecao >2003){							
			//Caso não exista edsital no ano atual, procura editais no ano anterior
				$anoSelecao = $anoSelecao-1;
				$qtdSelecao = $fSel->getSelecaoPorAno($pesqId,$anoSelecao);//correto
				//$qtdSelecao = $fSel->getSelecaoPorAno($this->getId());	//errado		
		}
		
		
		 if($qtdSelecao)  $numEditaisParticipando = count($qtdSelecao);					
		 else $numEditaisParticipando = -1;						

		 if($numEditaisParticipando>= 1 OR ($fPer->getPeriodoTipoAnoEdital('1',$_SESSION['sAno'],$_SESSION['sEdital'])== false) ){											
			/*if($tpSelecao > 0 and ($tpSelecao == $vetSelecao->getTipo()))*/ $_SESSION['sEdital'] = $editalSelec;	
			
		 }     
		 
		 return $numEditaisParticipando;
	}
	
	function descEdital($edital){
		$fTPEdital = new fachada_tipoedital();
		
		if($edital) $descEdital =$fTPEdital->getTipoEditalCod($edital);
		else $descEdital =$fTPEdital->getTipoEditalCod(1);
				
		if($descEdital) $nomeEdital = $descEdital->getDescricao();
		else $nomeEdital = "ERROR";
